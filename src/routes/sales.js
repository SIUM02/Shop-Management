...
function nextInvoiceNo() {
  ...
  const last = db
    .prepare('SELECT invoice_no FROM sales WHERE invoice_no LIKE ? ORDER BY id DESC LIMIT 1')
    .get(`${prefix}%`);
...
router.get(
  ...
  wrap((req, res) => {
    ...
    if (from)   { where.push('date(s.created_at) >= date(?)'); params.push(String(from)); }
    if (to)     { where.push('date(s.created_at) <= date(?)'); params.push(String(to)); }
  ...
  wrap((req, res) => {
    ...
    const sale = transaction(() => {
      ...
      const info = db
        ...
        .run(invoiceNo, customerName, customerPhone, subtotal, discount, tax,
      ...
      for (const item of priced) {
        insertItem.run(saleId, item.product_id, item.product_name, item.sku,
