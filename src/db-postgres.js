 ...
 */
/*
 * Schema changes that must also reach databases already in service.
 *
 * The setup block above only runs on a database that has no schema yet, so a
 * feature added later would otherwise arrive for new shops alone — the live
 * shop would keep serving the old shape. Each step here is applied once and
 * recorded in settings.schema_version.
 */
const MIGRATIONS = [
  ...
  [
    ...
    `ALTER TABLE sales ADD COLUMN IF NOT EXISTS paid_amount REAL NOT NULL DEFAULT 0`,
    /*
     * Every sale recorded before this feature existed was settled in full at
     * the counter — there was no way to record anything else. Leaving them at
     * the column default of 0 would invent a debt for every past customer.
     */
...
/**
 * Applies any migration the database has not seen, then records how far it got.
 * Statement errors that only mean "already there" are tolerated so a half-
 * applied step can be re-run.
 */
export async function applyMigrations() {
  const row = await q.get("SELECT value FROM settings WHERE key = 'schema_version'");
  let version = Number(row?.value || 0);

  for (let i = version; i < MIGRATIONS.length; i++) {
    for (const statement of MIGRATIONS[i]) {
      try {
        await q.exec(statement);
      } catch (err) {
        if (!/already exists|duplicate column|duplicate key/i.test(err.message)) throw err;
      }
    }
    version = i + 1;
  }
